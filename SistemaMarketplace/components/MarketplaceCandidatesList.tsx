...
const CandidateQuickView: React.FC<{ candidate: EnrichedCandidate | null; onClose: () => void }> = ({ candidate, onClose }) => {
  ...
  const analysis = {
    psychological_profile: candidate.psychologicalProfile || 'Perfil orientado a resultados, proactivo.' ,
    business_moment: candidate.businessMoment || 'En proceso de crecimiento profesional.',
    sales_angle: candidate.salesAngle || 'Ofrecer proyectos cortos para ganar experiencia.',
    bottleneck: candidate.bottleneck || 'Falta de proyectos completados o referencias.'
  };

  const invitation = candidate.walead_messages?.icebreaker || `Hola ${candidate.name}, me gustaría conectar y comentar oportunidades.`;
  const postAcceptance = candidate.walead_messages?.followup_message || `${candidate.name}, gracias por aceptar. ¿Tienes disponibilidad para una breve llamada?`;
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl w-full max-w-xl overflow-hidden shadow-2xl">
        <div className="px-4 py-3 border-b border-slate-800 flex justify-between items-center">
          <div>
            ...
            <p className="text-xs text-slate-400">{candidate.title} • {candidate.platform}</p>
          </div>
        ...
        <div className="p-4 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div className="p-3 bg-slate-800/40 rounded border border-slate-700">
              <p className="text-xs font-semibold text-slate-300">Perfil</p>
              <p className="text-sm text-slate-200 mt-1">{analysis.psychological_profile}</p>
            </div>
            <div className="p-3 bg-slate-800/40 rounded border border-slate-700">
              <p className="text-xs font-semibold text-slate-300">Momento</p>
              <p className="text-sm text-slate-200 mt-1">{analysis.business_moment}</p>
            </div>
            <div className="p-3 bg-slate-800/40 rounded border border-slate-700">
              <p className="text-xs font-semibold text-slate-300">Ángulo de Venta</p>
              <p className="text-sm text-slate-200 mt-1">{analysis.sales_angle}</p>
            </div>
            <div className="p-3 bg-slate-800/40 rounded border border-slate-700">
              <p className="text-xs font-semibold text-slate-300">Cuello de Botella</p>
              <p className="text-sm text-slate-200 mt-1">{analysis.bottleneck}</p>
            </div>
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div className="p-3 rounded bg-gradient-to-r from-blue-500/10 to-cyan-500/10 border border-blue-500/20">
              <p className="text-xs font-semibold text-blue-300">1️⃣ Invitación</p>
              <p className="text-sm text-slate-200 italic mt-2 min-h-[54px]">"{invitation}"</p>
              <button
            ...
            <div className="p-3 rounded bg-gradient-to-r from-emerald-500/10 to-teal-500/10 border border-emerald-500/20">
              <p className="text-xs font-semibold text-emerald-300">2️⃣ Post-aceptación</p>
              <p className="text-sm text-slate-200 italic mt-2 min-h-[54px]">"{postAcceptance}"</p>
              <button
                onClick={() => { navigator.clipboard.writeText(postAcceptance); }}
                className="mt-2 w-full px-3 py-2 bg-emerald-600 text-white rounded text-xs"
              >Copiar</button>
            </div>
          </div>
        </div>
